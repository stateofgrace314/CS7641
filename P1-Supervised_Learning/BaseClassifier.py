...
class BaseClassifier:
    ...
    def OptimizeParameters(self):
        ...
        out.to_csv(f"outputs/{self.prefix}ParameterOptimization_{self.dataset}.csv")
    ...
    def GenerateValidationCurve(self):
        ...
        for p, p_range in self.grid.items():
            ...
            i += 1
        # save and close
        plt.savefig(f"outputs/{self.prefix}ValidationCurve_{self.dataset}.png")
        plt.close()
    ...
    def GenerateLearningCurve(self):
        ...
        # save and close
        plt.savefig(f"outputs/{self.prefix}LearningCurve_{self.dataset}.png")
        plt.close()
